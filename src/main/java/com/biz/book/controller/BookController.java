...
public class BookController {
	 ...
	 * VO class를 만들어서 @ModelAttribute로 받는다.
	 * 
	 * spring에서는 form에서 보내는 데이터를 받을 때 
	 * form의 input box에 설정된 name 속성 값과 일치하는 
	 * 필드(맴버변수)들을 선언하고, getter와 setter, 생성자가 있는
	 * VO를 만들어서 ModelAttribute 변수로 사용하면 자동으로
	 * 해당 class를 객체로 선언하고, 각 필드에 값을 setting해서 
	 * 사용 할 수 있도록 구성한다.
	 * 
	 * input box에 입력되는 모든 값은 text(==String)이다
	 * 매개변수로 값을 받을 때 String이 아닌 다른 type으로 받으면
	 * String to type의 형변환을 시도한다.(공란일 경우에도 Exception)
	 * ex) VO의 b_price가 int형이면 
	 * 		실제로는 setB_price(Integer.valueOf(b_price))
	 * 		형식의 코드가 실행 될 것이다.
